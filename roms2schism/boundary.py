...
def make_boundary(schism, template, dates, dcrit = 700,
    ...
    for it in progressbar(range(0, nt)):
        ...
        schism_zeta[it,:,0,0] = itp.interp2D(roms_data.zeta[it, mask_OK], interp, dcrit)
        # compute depths for each ROMS levels at the specific SCHISM locations
        roms_depths_at_schism_node = rs.roms_depth_point(schism_zeta[it,:,0,0], depth_interp,
        ...
        for k in range(0, Nz):   
            temp_interp[k,:] = itp.interp2D(roms_data.temp[it,k,][mask_OK], interp, dcrit)
        ...
        for k in range(0,Nz):
            temp_interp[k,:] = itp.interp2D(roms_data.salt[it,k,][mask_OK], interp, dcrit)
        ...
        temp_interp = np.zeros((Nz, schism.NOP))
        for k in range(0,Nz):
            temp_interp[k,:] = itp.interp2D(roms_data.u[it,k,][mask_OK], interp, dcrit)
        # now you need to interp temp for each NOP at SCHISM depths
        ...
        temp_interp = np.zeros((Nz, schism.NOP))
        for k in range(0,Nz):
            temp_interp[k,:] = itp.interp2D(roms_data.v[it,k,][mask_OK], interp, dcrit)
        # now you need to interp temp for each NOP at SCHISM depths
